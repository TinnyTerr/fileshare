...
function getMasterKey(): Buffer {
  ...
  return createHash('sha256').update('master:' + fallback).digest();
}
...
function aesgcmEncrypt(data: Buffer, key: Buffer): { ciphertext: Buffer; iv: Buffer; tag: Buffer } {
...
export function wrapKey(fileKey: Buffer): { wrapped: string; iv: string; tag: string } {
...
export function unwrapKeyWithPassword(
  ...
  const derivedKey = pbkdf2Sync(password, Buffer.from(salt, 'hex'), 600_000, 32, 'sha256');
  return aesgcmDecrypt(
    Buffer.from(wrapped, 'hex'),
    derivedKey,
    Buffer.from(iv, 'hex'),
