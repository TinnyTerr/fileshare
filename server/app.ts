...
function cors(res: Response): Response {
	...
	res.headers.set(
		...
		"Content-Type, Authorization, Content-Range, X-Deploy-Key, Range, X-Share-Password",
	);
	res.headers.set(
		"Access-Control-Expose-Headers",
		"Content-Range, Accept-Ranges, Content-Length, X-SHA256, X-Chunked-Download",
	);
	return res;
}
...
async function route(
	...
	if (fileMatch) {
		...
		if (method === "PUT" && sub === "/data")
			return handleUploadChunk(req, fileId);
		if (method === "DELETE" && sub === "") return handleDeleteFile(req, fileId);
		if (method === "POST" && sub === "/share")
			return handleShareFile(req, fileId);
		if (method === "DELETE" && sub === "/share")
			return handleUnshareFile(req, fileId);
		if (method === "POST" && sub === "/share-group")
			return handleShareGroup(req, fileId);
		// DELETE /api/files/:id/share-group/:groupId
		const groupShareMatch = sub.match(/^\/share-group\/(\d+)$/);
		if (method === "DELETE" && groupShareMatch)
			return handleUnshareGroup(req, fileId, groupShareMatch[1]!);
		// Share links for this file
		if (method === "POST" && sub === "/links")
			return handleCreateLink(req, fileId);
		if (method === "GET" && sub === "/links")
			return handleListLinks(req, fileId);
	...
	if (userMatch) {
		...
		if (method === "PUT" && sub === "/subscription")
			return handleUpdateSubscription(req, userId);
		if (method === "PUT" && sub === "/role")
			return handleUpdateRole(req, userId);
	}
	...
	if (groupMatch) {
		...
		if (method === "DELETE" && sub === "")
			return handleDeleteGroup(req, groupId);
		if (method === "POST" && sub === "/members")
			return handleAddMember(req, groupId);
		if (method === "DELETE" && sub === "/leave")
			return handleLeaveGroup(req, groupId);
		const memberMatch = sub.match(/^\/members\/(\d+)$/);
		if (method === "DELETE" && memberMatch)
			return handleRemoveMember(req, groupId, memberMatch[1]!);
		if (method === "POST" && sub === "/invites")
			return handleCreateInvite(req, groupId);
		if (method === "GET" && sub === "/invites")
			return handleListInvites(req, groupId);
		const inviteMatch = sub.match(/^\/invites\/([^/]+)$/);
		if (method === "DELETE" && inviteMatch)
			return handleRevokeInvite(req, groupId, inviteMatch[1]!);
	}

	const joinMatch = path.match(/^\/api\/groups\/join\/([^/]+)$/);
	if (joinMatch && method === "POST")
		return handleJoinGroup(req, joinMatch[1]!);
	...
	if (dlDataMatch) {
		if (method === "GET" || method === "POST")
			return handleDownloadViaLink(req, dlDataMatch[1]!);
	}
	...
	if (method === "GET" && path === "/api/version")
		return handleGetLatestVersion(req);
	if (method === "GET" && path === "/api/version/history")
		return handleListVersions(req);
	if (method === "POST" && path === "/api/version")
		return handlePublishVersion(req);

	const vdlMatch = path.match(/^\/api\/version\/download\/([^/]+)\/([^/]+)$/);
	if (vdlMatch && method === "GET")
		return handleDownloadClient(req, vdlMatch[1]!, vdlMatch[2]!);
