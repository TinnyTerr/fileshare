...
import { unwrapKey, wrapKeyWithPassword, unwrapKeyWithPassword } from '../crypto';
import { serveFile } from './files';

export async function handleCreateLink(req: Request, fileId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;

  const file = db.query('SELECT owner_id, key_wrapped, key_iv, key_tag FROM files WHERE id = ?').get(fileId) as any;
  ...
  db.query(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(id, fileId, auth.userId, can_write ? 1 : 0, max_downloads ?? null, expiresAt,
    passKeyWrapped, passKeyIv, passKeyTag, passSalt);
...
export async function handleListLinks(req: Request, fileId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;
...
export async function handleRevokeLink(req: Request, fileId: string, linkId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;
  ...
  db.query('DELETE FROM share_links WHERE id = ? AND file_id = ?').run(linkId, fileId);
  return json({ ok: true });
}

export async function handleAccessLink(req: Request, linkId: string): Promise<Response> {
  const link = db.query(`
  ...
  if (link.max_downloads !== null && link.downloads >= link.max_downloads) return json({ error: 'Download limit reached' }, 410);
  if (link.file_expires_at && new Date(link.file_expires_at) < new Date()) return json({ error: 'File expired' }, 410);
...
export async function handleDownloadViaLink(req: Request, linkId: string): Promise<Response> {
  const link = db.query('SELECT * FROM share_links WHERE id = ?').get(linkId) as any;
  if (!link) return json({ error: 'Link not found' }, 404);
  if (link.expires_at && new Date(link.expires_at) < new Date()) return json({ error: 'Link expired' }, 410);
  if (link.max_downloads !== null && link.downloads >= link.max_downloads) return json({ error: 'Download limit reached' }, 410);
  ...
  if (link.pass_salt) {
    ...
    const password = req.headers.get('X-Share-Password') ||
      (req.method === 'POST' ? (await req.json().catch(() => ({} as any))).password : null);

    if (!password) {
      return json({ error: 'password required', code: 'PASSWORD_REQUIRED' }, 401);
    }
    try {
      overrideFileKey = unwrapKeyWithPassword(
        link.pass_key_wrapped, link.pass_key_iv, link.pass_key_tag, link.pass_salt, password
      );
    } catch {
  ...
  db.query('UPDATE share_links SET downloads = downloads + 1 WHERE id = ?').run(linkId);
