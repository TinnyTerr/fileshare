...
import { generateFileKey, wrapKey, unwrapKey, encryptChunk, decryptChunk, CHUNK_HEADER_SIZE } from '../crypto';
...
function fileDir(fileId: string)  { return join(UPLOAD_DIR, fileId.slice(0, 2), fileId.slice(2, 4)); }
function filePath(fileId: string) { return join(fileDir(fileId), fileId); }
function tmpDir(fileId: string)   { return join(TMP_DIR, fileId); }
function chunkPath(fileId: string, start: number) { return join(tmpDir(fileId), `chunk_${start}`); }
...
function canRead(fileId: string, userId: number, role: string): boolean {
  ...
  const file = db.query('SELECT owner_id FROM files WHERE id = ?').get(fileId) as any;
  if (!file) return false;
  if (file.owner_id === userId) return true;
  if (db.query('SELECT 1 FROM file_shares WHERE file_id = ? AND user_id = ?').get(fileId, userId)) return true;
  if (db.query(`
    SELECT 1 FROM file_group_shares fgs
    JOIN group_members gm ON gm.group_id = fgs.group_id
    WHERE fgs.file_id = ? AND gm.user_id = ?
  `).get(fileId, userId)) return true;
  return false;
}
...
function canWrite(fileId: string, userId: number, role: string): boolean {
  ...
  if (!file) return false;
  if (file.owner_id === userId) return true;
  const share = db.query('SELECT can_write FROM file_shares WHERE file_id = ? AND user_id = ?').get(fileId, userId) as any;
  if (share?.can_write) return true;
  const groupShare = db.query(`
...
async function assembleAndEncrypt(fileId: string, totalSize: number, fileKey: Buffer): Promise<void> {
  const dir = fileDir(fileId);
  mkdirSync(dir, { recursive: true });

  const { openSync, writeSync, closeSync, readFileSync, rmSync } = await import('fs');
  ...
  while (cleartextOffset < totalSize) {
    ...
    insertChunk.run(fileId, chunkIndex, cleartextOffset, data.length, encryptedOffset);
...
export async function handleInitUpload(req: Request): Promise<Response> {
  ...
  const { filename, size, mime_type = 'application/octet-stream', expires_days } = body;
  ...
  ).run(fileId, auth.userId, filename, size, mime_type, wrapped, iv, tag, expiresAt);
...
export async function handleUploadChunk(req: Request, fileId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;

  const file = db.query('SELECT * FROM files WHERE id = ?').get(fileId) as any;
  if (!file) return json({ error: 'File not found' }, 404);
  if (!canWrite(fileId, auth.userId, auth.role)) return json({ error: 'Forbidden' }, 403);
  if (file.status === 'complete') return json({ error: 'Upload already complete' }, 409);

  const contentRange = req.headers.get('Content-Range');
  if (!contentRange) return json({ error: 'Content-Range header required' }, 400);

  const match = contentRange.match(/^bytes (\d+)-(\d+)\/(\d+)$/);
  if (!match) return json({ error: 'Invalid Content-Range. Expected: bytes start-end/total' }, 400);
  ...
  if (chunkSize > CHUNK_SIZE) {
    return json({ error: `Chunk exceeds 100MB limit (got ${Math.ceil(chunkSize / 1024 ** 2)}MB)`, code: 'CHUNK_TOO_LARGE' }, 413);
  }
  if (total !== file.size) return json({ error: 'Total size mismatch' }, 400);

  const body = await req.arrayBuffer();
  if (body.byteLength !== chunkSize) return json({ error: 'Body size does not match Content-Range' }, 400);

  await Bun.write(chunkPath(fileId, start), body);

  const newBytesReceived = file.bytes_received + chunkSize;
  db.query('UPDATE files SET bytes_received = ? WHERE id = ?').run(newBytesReceived, fileId);
  ...
  if (newBytesReceived >= file.size) {
    ...
    db.query("UPDATE files SET status = 'complete', bytes_received = size WHERE id = ?").run(fileId);
    db.query('UPDATE users SET storage_used = storage_used + ? WHERE id = ?').run(file.size, file.owner_id);

    const complete = db.query('SELECT id, filename, size, mime_type, status, created_at, expires_at FROM files WHERE id = ?').get(fileId);
    return json({ complete: true, file: complete });
  }

  return json({ complete: false, bytes_received: newBytesReceived, bytes_remaining: file.size - newBytesReceived }, 202);
}

export async function handleDownload(req: Request, fileId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;
...
export async function serveFile(
  ...
  const file = db.query('SELECT * FROM files WHERE id = ?').get(fileId) as any;
  if (!file) return json({ error: 'File not found' }, 404);
  if (file.status !== 'complete') return json({ error: 'Upload incomplete' }, 409);
  if (file.expires_at && new Date(file.expires_at) < new Date()) return json({ error: 'File expired' }, 410);

  if (!overrideFileKey && !canRead(fileId, userId, role)) return json({ error: 'Forbidden' }, 403);

  const path = filePath(fileId);
  if (!existsSync(path)) return json({ error: 'File data missing' }, 404);

  const fileKey = overrideFileKey ?? unwrapKey(file.key_wrapped, file.key_iv, file.key_tag);
  ...
  if (rangeHeader) {
    ...
    const requestedEnd = match[2] ? parseInt(match[2]) : reqStart + CHUNK_SIZE - 1;

    if (requestedEnd - reqStart + 1 > CHUNK_SIZE) {
      return json({ error: 'Range exceeds 100MB. Request at most 100MB per request.', code: 'RANGE_TOO_LARGE' }, 413);
    }
  ...
  for (const chunk of chunks) {
    ...
    const encBytes = await encFile.slice(chunk.encrypted_offset, chunk.encrypted_offset + encSize).arrayBuffer();
    const cleartext = decryptChunk(Buffer.from(encBytes), fileKey);

    const sliceFrom = Math.max(0, reqStart - chunk.cleartext_start);
    const sliceTo   = Math.min(chunk.cleartext_size, reqEnd - chunk.cleartext_start + 1);
...
export async function handleGetFileMeta(req: Request, fileId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;

  const file = db.query('SELECT id, owner_id, filename, size, mime_type, status, bytes_received, created_at, expires_at FROM files WHERE id = ?').get(fileId) as any;
  if (!file) return json({ error: 'File not found' }, 404);
...
export async function handleListFiles(req: Request): Promise<Response> {
  ...
  const total = (db.query("SELECT COUNT(*) as c FROM files WHERE owner_id = ? AND status = 'complete'").get(auth.userId) as any).c;
  return json({ files: owned, shared, group_shared: groupShared, total, page, limit });
}

export async function handleDeleteFile(req: Request, fileId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;
  ...
  if (file.status === 'complete') {
    db.query('UPDATE users SET storage_used = MAX(0, storage_used - ?) WHERE id = ?').run(file.size, file.owner_id);
...
export async function handleShareFile(req: Request, fileId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;
...
export async function handleShareGroup(req: Request, fileId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;
  ...
  const group = db.query('SELECT id FROM groups WHERE id = ?').get(body.group_id) as any;
...
export async function handleUnshareGroup(req: Request, fileId: string, groupId: string): Promise<Response> {
  const auth = await requireAuth(req);
  if (isResponse(auth)) return auth;
  ...
  db.query('DELETE FROM file_group_shares WHERE file_id = ? AND group_id = ?').run(fileId, groupId);
  return json({ ok: true });
}
