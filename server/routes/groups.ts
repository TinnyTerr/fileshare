...
export async function handleGetGroup(
	...
	if (!isMember(gid, auth.userId) && auth.role !== "admin")
...
export async function handleUpdateGroup(
	...
	const gid = parseInt(groupId);
	if (!isGroupAdmin(gid, auth.userId) && auth.role !== "admin")
		return json({ error: "Forbidden" }, 403);

	const body = await req.json().catch(() => null);
	if (!body?.name?.trim()) return json({ error: "name required" }, 400);

	db.query("UPDATE groups SET name = ? WHERE id = ?").run(
		body.name.trim(),
		gid,
	);
...
export async function handleDeleteGroup(
	...
	const group = db
		...
		.get(gid) as any;
	if (!group) return json({ error: "Group not found" }, 404);
	if (group.owner_id !== auth.userId && auth.role !== "admin")
		return json({ error: "Forbidden" }, 403);
...
export async function handleRemoveMember(
	...
	if (uid === group.owner_id)
		return json({ error: "Cannot remove group owner" }, 400);

	db.query("DELETE FROM group_members WHERE group_id = ? AND user_id = ?").run(
		gid,
		uid,
	);
	return json({ ok: true });
...
export async function handleCreateInvite(
	...
	return json(
		{
			id,
			invite_url: `${serverUrl}/api/groups/join/${id}`,
...
export async function handleRevokeInvite(
	...
	db.query("DELETE FROM group_invites WHERE id = ? AND group_id = ?").run(
		inviteId,
		gid,
	);
	return json({ ok: true });
...
export async function handleJoinGroup(
	...
	if (invite.pass_hash) {
		...
		if (!password)
			return json(
				{
					error: "password required for this invite",
					code: "PASSWORD_REQUIRED",
				},
				401,
			);
		if (!(await Bun.password.verify(password, invite.pass_hash)))
			return json({ error: "Wrong invite password" }, 401);
	}

	// Already a member?
	if (isMember(invite.group_id, auth.userId))
		return json({ error: "Already a member" }, 409);

	db.query(
		"INSERT INTO group_members (group_id, user_id, role) VALUES (?, ?, 'member')",
	).run(invite.group_id, auth.userId);
	db.query("UPDATE group_invites SET uses = uses + 1 WHERE id = ?").run(
		inviteId,
	);
