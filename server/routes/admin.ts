...
export async function handleUpdateSubscription(
	...
	if (!tier || !Object.keys(SUBSCRIPTION_LIMITS).includes(tier)) {
		return json(
			{
				error: `tier must be one of: ${Object.keys(SUBSCRIPTION_LIMITS).join(", ")}`,
			},
			400,
		);
	}
	...
	db.query("UPDATE users SET subscription_tier = ? WHERE id = ?").run(
		tier,
		userId,
	);
...
export async function handleStats(_req: Request): Promise<Response> {
	...
	const fileCount = (
		db
			.query("SELECT COUNT(*) as c FROM files WHERE status = 'complete'")
			.get() as any
	).c;
	const totalStorage = (
		db
			.query("SELECT COALESCE(SUM(storage_used), 0) as s FROM users")
			.get() as any
	).s;
