import { api, uploadFile, downloadFile, ApiError } from '../api';
...
export async function cmdUpload(args: string[]): Promise<void> {
  ...
  if (!localPath) { console.error('Usage: fileshare upload <path> [--expires <days>]'); process.exit(1); }
  if (!existsSync(localPath)) { console.error(`File not found: ${localPath}`); process.exit(1); }

  const expiresIdx = args.indexOf('--expires');
  const expiresDays = expiresIdx >= 0 ? parseInt(args[expiresIdx + 1]!) : undefined;

  const fileInfo = Bun.file(localPath);
  console.log(`Uploading: ${localPath.split('/').pop()} (${formatBytes(fileInfo.size)})`);
  ...
  try {
    ...
    if (err instanceof ApiError && err.code === 'QUOTA_EXCEEDED') {
      console.error(`Upload failed: Storage quota exceeded. Upgrade your plan.`);
    } else {
...
export async function cmdDownload(args: string[]): Promise<void> {
  const fileId = args[0];
  if (!fileId) { console.error('Usage: fileshare download <file-id> [--output <path>]'); process.exit(1); }
  ...
  try {
    ...
    if (data.files.length > 0) {
      ...
      for (const f of data.files) {
        const exp = f.expires_at ? ` [expires ${f.expires_at.slice(0, 10)}]` : '';
        console.log(`  ${f.id}  ${f.filename.padEnd(30)} ${formatBytes(f.size).padStart(10)}${exp}`);
      }
    ...
    if (data.shared.length > 0) {
      ...
      for (const f of data.shared) {
        ...
        console.log(`  ${f.id}  ${f.filename.padEnd(30)} ${formatBytes(f.size).padStart(10)} ${rw}`);
      }
    }
  } catch (err: any) {
...
export async function cmdInfo(args: string[]): Promise<void> {
  const fileId = args[0];
  if (!fileId) { console.error('Usage: fileshare info <file-id>'); process.exit(1); }
...
export async function cmdDelete(args: string[]): Promise<void> {
  const fileId = args[0];
  if (!fileId) { console.error('Usage: fileshare delete <file-id>'); process.exit(1); }
