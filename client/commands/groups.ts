...
export async function cmdGroupCreate(args: string[]): Promise<void> {
  ...
  if (!name) { console.error('Usage: fileshare group create <name>'); process.exit(1); }
...
export async function cmdGroupList(): Promise<void> {
  try {
    ...
    if (!groups.length) { console.log('No groups yet.'); return; }
    console.log('\nGroups:');
    console.log('─'.repeat(60));
    for (const g of groups) {
      console.log(`  [${g.id}] ${g.name.padEnd(24)} ${g.role.padEnd(8)} ${g.member_count} members`);
    }
  } catch (err: any) {
    console.error(`Failed: ${err.message}`); process.exit(1);
  }
...
export async function cmdGroupInfo(args: string[]): Promise<void> {
  const id = args[0];
  if (!id) { console.error('Usage: fileshare group info <id>'); process.exit(1); }
  try {
...
export async function cmdGroupRemoveMemberById(args: string[]): Promise<void> {
  ...
  if (!groupId || !userId) {
    console.error('Usage: fileshare group remove-member <group-id> --user-id <user-id>');
    process.exit(1);
  }
  try {
...
export async function cmdGroupInvite(args: string[]): Promise<void> {
  const groupId = args[0];
  if (!groupId) { console.error('Usage: fileshare group invite <group-id> [--password <pw>] [--expires <days>] [--max-uses <n>]'); process.exit(1); }
  ...
  try {
    const result = await api.groups.createInvite(parseInt(groupId), { password, expires_days: expiresDays, max_uses: maxUses }) as any;
...
export async function cmdGroupJoin(args: string[]): Promise<void> {
  const token = args[0];
  if (!token) { console.error('Usage: fileshare group join <invite-token> [--password <pw>]'); process.exit(1); }
  ...
  try {
    ...
    if (err.code === 'PASSWORD_REQUIRED') {
      console.error('This invite requires a password. Use: fileshare group join <token> --password <pw>');
    } else {
...
export async function cmdGroupLeave(args: string[]): Promise<void> {
  const groupId = args[0];
  if (!groupId) { console.error('Usage: fileshare group leave <group-id>'); process.exit(1); }
  try {
...
export async function cmdGroupDelete(args: string[]): Promise<void> {
  const groupId = args[0];
  if (!groupId) { console.error('Usage: fileshare group delete <group-id>'); process.exit(1); }
  try {
