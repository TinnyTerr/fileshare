...
export async function cmdUpdate(args: string[]): Promise<void> {
  ...
  try {
    ...
    console.error(`Update check failed: ${err.message}`); process.exit(1);
  }

  const latest = info.version as string;
  if (!force && !semverGt(latest, cfg.client_version)) {
    console.log(`Already up to date (${cfg.client_version})`); return;
  }

  console.log(`Downloading ${cfg.client_version} → ${latest}...`);

  const res = await fetch(info.download_url);
  if (!res.ok) { console.error(`Download failed: ${res.statusText}`); process.exit(1); }
  ...
  if (actualHash !== info.sha256) {
    console.error('SHA256 mismatch — update rejected. Binary may be corrupted or tampered.');
    process.exit(1);
  }
  ...
  if (process.platform === 'win32') {
    ...
    console.log(`✓ Updated to ${latest}. Restart fileshare to use the new version.`);
  }
}

// On Windows, you can't replace a running executable.
// Write a .bat that waits for this process to exit, then swaps the files.
async function replaceOnWindows(selfPath: string, tmpPath: string, newVersion: string): Promise<void> {
  ...
  spawn('cmd', ['/c', 'start', '', '/min', batPath], { detached: true, stdio: 'ignore' }).unref();

  saveConfig({ client_version: newVersion });
  console.log(`✓ Updater launched. The binary will be replaced after you exit.`);
