...
export async function cmdCreateLink(args: string[]): Promise<void> {
	...
	if (!fileId) {
		console.error(
			"Usage: fileshare link create <file-id> [--password <pw>] [--expires <days>] [--max-downloads <n>] [--write]",
		);
		process.exit(1);
	}
	...
	try {
		const result = (await api.links.create(fileId, {
			password,
		...
		if (result.password_protected)
			console.log(
				`  Password protected (E2E — server cannot decrypt without password)`,
			);
		if (result.expires_at) console.log(`  Expires: ${result.expires_at}`);
		if (result.max_downloads)
			console.log(`  Max downloads: ${result.max_downloads}`);
	} catch (err: any) {
...
export async function cmdListLinks(args: string[]): Promise<void> {
	const fileId = args[0];
	if (!fileId) {
		console.error("Usage: fileshare link list <file-id>");
		process.exit(1);
	}
	...
	try {
		...
		if (!links.length) {
			console.log("No active share links.");
			return;
		}
		...
		for (const l of links) {
			...
			const dl = l.max_downloads
				? `${l.downloads}/${l.max_downloads} dl`
				: `${l.downloads} dl`;
...
export async function cmdDownloadLink(args: string[]): Promise<void> {
	...
	if (!token) {
		console.error(
			"Usage: fileshare link download <token> [--password <pw>] [--output <path>]",
		);
		process.exit(1);
	}
	...
	try {
		...
		console.error(`Failed to fetch link info: ${err.message}`);
		process.exit(1);
	}

	if (info.error) {
		console.error(`Link error: ${info.error}`);
		process.exit(1);
	}
	...
	while (offset < info.size) {
		...
		const res = await fetch(`${cfg.server}/api/dl/${token}/data`, {
		...
		process.stdout.write(
			`\r[${progressBar(Math.round((offset / info.size) * 100))}] ${formatBytes(offset)}`,
		);
	}
